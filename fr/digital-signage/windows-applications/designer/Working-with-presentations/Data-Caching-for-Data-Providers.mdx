PADS4 Designer inclut **la mise en cache des données** fonctionnalité pour les fournisseurs de données, facilitant et accélérant la conception de présentations pilotées par les données — même lorsque la source de données est lente ou temporairement indisponible.
...
* **Les visionneuses PADS4 N'utilisent PAS les données mises en cache.** Elles récupèrent toujours **données en direct** à l'exécution.
...
| **Actualisation automatique des données** | Les données mises en cache se chargent d'abord, l'actualisation en arrière-plan se produit une fois        |
| **Partage avec des tiers**                | Les données mises en cache permettent à d'autres de concevoir sans accéder aux données en direct           |
| **Comportement du visualiseur**           | **Les visualiseurs n'utilisent pas les données mises en cache** ; ils nécessitent des connexions en direct |
