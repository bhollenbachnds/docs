...
Pour plus de détails sur **la liaison d'un élément** à une source de données externe, référez-vous aux **propriétés du fournisseur de données** et **travailler avec les fournisseurs de données**.
...
Si vous avez besoin de stocker des **playlists imbriquées** dans un **fichier XML**, l'analyseur XML peut mal interpréter la structure. Pour éviter cela, **remplacez** `<` et `>` par `&lt;` et `&gt;`.
...
| **999** | Effet par défaut (utilise **Effet de transition par défaut** propriété) |

***

### **Notes finales**

* Utilisation de **playlists externes** garantit **des mises à jour de contenu dynamiques** sans intervention manuelle.
