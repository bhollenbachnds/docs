Cette section fournit des instructions étape par étape pour créer la base de données **PADS4FIDS** à l'aide de **PowerShell**, destinée aux administrateurs système ayant accès à SQL Server.

> **Important:** Exécutez ces commandes sur un système avec **SQL Server installé**, en utilisant un compte avec des **privilèges d'administrateur système** sur l'instance SQL.
...
* Une nouvelle base de données **PADS4FIDS** est créée (si elle n'existe pas déjà)
* La connexion se voit accorder les privilèges **db\_owner** sur la base de données
