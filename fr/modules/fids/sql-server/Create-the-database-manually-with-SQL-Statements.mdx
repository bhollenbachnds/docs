Cette section fournit des instructions basées sur SQL pour créer manuellement la base de données **PADS4FIDS** en utilisant votre client SQL préféré (par ex., SQL Server Management Studio, Azure Data Studio, etc.).

> **Note:** Exécutez les commandes SQL suivantes en utilisant un compte avec des **privilèges sysadmin** sur le SQL Server.
