<Warning>
  À partir de PADS4 2025.2, cette fonctionnalité fait partie de PADS4 CTRL Center. Pour plus d'informations, consultez CTRL Center > Administration > Identity.
</Warning>
...
| **URL du service consommateur d'assertions** | `https://dpads4serverurl/crystal/domainone/Saml2/ACS` |
| **URL SSO du fournisseur de services**       | `https://pads4serverurl/crystal/domainone/Saml2/ACS`  |
...
Laisser **Certificat du fournisseur de services** et **Options de chiffrement** vide sauf si le chiffrement est requis par votre organisation.
...
| **URL des métadonnées de fédération** (facultatif)           | `https://pads4.verify.ibm.com/v1.0/saml/federations/saml2`    |
...
| **Certificat de signature de la partie de confiance (.pfx)** | Télécharger votre `.pfx` fichier avec clé privée              |
| **Mot de passe de la clé privée**                            | Entrer le mot de passe pour `.pfx`                            |

***

## Étape 3 : Mappage des groupes dans PADS4

Accéder à la **Table de relations** section du plugin SSO et mapper les `groupIDs` entrants d'IBM vers les profils PADS4 :
...
* Tester la connexion via PADS4 avec un utilisateur vérifié par IBM.
* S'assurer que les revendications sont correctement reçues et analysées dans PADS4.
* Ajuster le mappage des attributs ou les règles de revendication si les utilisateurs ne sont pas correctement assignés.
