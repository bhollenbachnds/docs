...
* URL des métadonnées de fédération (optionnel si importé manuellement)
...
| **Certificat du serveur de fédération**                      | Téléverser le **certificat de signature IBM Verify (.cer)**   |
| **URL de la partie de confiance**                            | `https://pads4serverurl/crystal/domainone`                    |
| **Certificat de signature de la partie de confiance (.pfx)** | Téléverser votre `.pfx` fichier avec clé privée               |
...
Accéder à la **Relation Table** section du plugin SSO et mapper les `groupIDs` d'IBM vers les profils PADS4 :
