...
Pour que SSO fonctionne correctement, PADS4 doit être configuré avec HTTPS. Pour ce faire, vous devrez suivre ce guide en utilisant un certificat SSL.
...
| App Federation Metadata           | URL de métadonnées de fédération           |
| Certificat (Brut)                 | Certificat du serveur de fédération (.cer) |
| ID d'objet                        | ID de groupe Active Directory              |

### Important :

La chaîne copiée pour le **" URL de la partie de confiance "** dans la configuration SSO du CMS ne doit pas contenir\*\*" /Saml "\*\* tel que copié depuis Azure, mais uniquement\*\*" /crystal/pads "\*\* .
...
Pour le **"certificat de signature de la partie de confiance"**, nous vous suggérons d'utiliser le certificat que vous utilisez pour votre configuration HTTPS. "Convertissez ce certificat en .pfx."
