...
Ce manuel ne peut être suivi que si vous avez configuré le SSO dans votre environnement Azure / Entra ID.

Maintenant que toutes les informations ci-dessus ont été recueillies, PADS4CMS peut être configuré pour utiliser le SSO.

## Important :

Pour que le SSO fonctionne correctement, PADS4 doit être configuré avec l'utilisation de HTTPS. Pour y parvenir, vous devrez suivre ce guide en utilisant un certificat SSL.
...
Activez le plugin en **haut à droite**, à côté de **"Type"**, sélectionnez l'**"Azure AD"** option dans la liste déroulante
...
La chaîne copiée pour le **" URL of the relying party "** dans la configuration SSO du CMS ne doit pas contenir\*\*" /Saml "\*\* tel que copié depuis Azure, mais uniquement\*\*" /crystal/pads "\*\* .
...
Lorsque vous configurez le\*\*"Relation Table"\*\* dans CMS Admin pour le SSO, définissez un profil que vous avez créé pour les utilisateurs ou groupes SSO en relation avec leur niveau de privilège.
