Ce manuel ne peut être suivi que si vous avez configuré SSO dans votre environnement Azure / Entra ID. Vous pouvez suivre cet article pour ce faire : **Single Sign On (Azure)**
...
Pour que SSO fonctionne correctement, PADS4 doit être configuré avec HTTPS. Pour y parvenir, vous devrez suivre ce guide en utilisant un certificat SSL.
...
Pour le **"certificat de signature de la partie de confiance"**, nous suggérons d'utiliser le certificat que vous utilisez pour votre configuration HTTPS. "Convertissez ce certificat en .pfx."
...
Lorsque vous configurez le \*\*"Tableau de relation"\*\* dans CMS Admin pour SSO, définissez un profil que vous avez créé pour les utilisateurs ou groupes SSO en relation avec leur niveau de privilège.
