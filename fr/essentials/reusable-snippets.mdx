...
1. Ajoutez du contenu à votre fichier d'extrait que vous souhaitez réutiliser à travers plusieurs
   emplacements. Facultativement, vous pouvez ajouter des variables qui peuvent être remplies via des props
...
<Warning>
  Le contenu que vous souhaitez réutiliser doit être à l'intérieur du répertoire `snippets` pour
...
<Warning>
  MDX ne se compile pas à l'intérieur du corps d'une fonction fléchée. Tenez-vous en à la syntaxe HTML
  quand vous le pouvez ou utilisez une exportation par défaut si vous avez besoin d'utiliser MDX.
</Warning>
