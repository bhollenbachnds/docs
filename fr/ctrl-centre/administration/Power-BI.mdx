...
* **Seule l'intégration de rapports et de tableaux de bord est prise en charge**.
* **Les utilisateurs ne peuvent pas se connecter** au portail Power BI en utilisant un principal de service.
* **Les droits d'administrateur Power BI sont nécessaires** pour activer le principal de service dans les **Developer settings**.
...
Pour intégrer Power BI à PADS4, un **new App Registration** est nécessaire dans **Microsoft Azure**.
...
| --------------------------- | ------------------------------------------- |
| **Application (Client) ID** | Aperçu de l'enregistrement de l'application |
| **Directory (Tenant) ID**   | Aperçu de l'enregistrement de l'application |
| **Client Secret**           | Certificats et secrets                      |
...
5. **Cliquez** **Add People or Groups**.
6. **Recherchez** le **nom d'enregistrement de l'application** (créé dans Azure).
...
## **5. Récupérer les ID d'espace de travail Power BI et de rapport/tableau de bord**
...
* **Port**: Facultatif (à spécifier uniquement si vous n'utilisez pas les ports HTTP/HTTPS par défaut).
* **ID de jeton**: Généré dans **Credential Manager**.
...
Une fois terminé, **PADS4 est maintenant intégré avec succès à Power BI** et prêt à afficher **Power BI Reports/Dashboards**.
