...
1. **Connectez-vous au** **Power BI Admin portal**.
...
3. **Trouvez le Workspace** que vous souhaitez utiliser et **ouvrez More Options**.
...
## **5. Récupérer les ID de l'espace de travail Power BI et des rapports/tableaux de bord**

Pour afficher le contenu Power BI dans PADS4, vous avez besoin du **Workspace ID** et de l'**Report/Dashboard ID**.

### **Trouvez les ID dans l'URL Power BI**

1. **Ouvrez** le **Power BI report or dashboard**.
...
* **Workspace ID** : Trouvé **between** `groups/` **and** `/reports`.
* **Report ID** : Trouvé **between** `/reports/` **and** `/ReportSection`.
...
* **Workspace ID** : Trouvé **between** `groups/` **and** `/dashboards`.
* **Dashboard ID** : Trouvé **after** `/dashboards/`.
...
2. **Ouvrez** **Credentials** sous le **Administration menu**.
...
2. **Trouvez l'App Registration que vous avez ajouté**.
...
* **Protocol** : `http` ou `https` (utilisez `https` si vous utilisez un certificat SSL public valide).
