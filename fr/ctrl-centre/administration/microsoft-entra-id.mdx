...
## Créer une Application Entreprise Microsoft Azure
...
## Modifier les Attributes & Claims

1. Vous devez modifier les **Attributes & Claims**. Cliquez sur **Edit** pour le faire.
...
Pour spécifier quels groupes d'utilisateurs peuvent accéder à l'application, attribuez les groupes d'utilisateurs appropriés.
...
1. **App Federation Metadata URL** (par exemple, `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/federationmetadata/2007-06/federationmetadata.xml?appid=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
2. **Certificate (Raw)** – Cliquez sur le lien pour le télécharger (par exemple, `your-application-name.cer`)
3. **Login URL** (par exemple, `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/saml2`)
4. **Microsoft Entra Identifier** (par exemple, `https://sts.windows.net/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/`)
...
1. Connectez-vous à PADS4 et naviguez vers Adminstration
...
2. **Identifier** – Entrez le \_Microsoft Entra Identifier \_que vous avez collecté précédemment
3. **Reply URL** – Entrez l'URL de votre application PADS4 locale (par exemple, `https://pads4.mycompany.com`).
4. **Metadata URL** – Entrez l'URL de métadonnées Microsoft Entra : \_App Federation Metadata URL \_que vous avez collectée.
...
5. Le \*\*Identifier \*\*et **Assertion Consumer Service URL** sont remplis.
...
2. **Certificat du fournisseur d'identité PADS4** - Téléchargez un certificat (`.pfx`) pour signer les assertions SAML de PADS4 vers Microsoft Entra.
3. \*\*Mot de passe \*\*- Saisissez le mot de passe pour le `.pfx` certificat téléchargé (sous le champ du certificat).
...
Si vous n'avez pas besoin d'activer le provisionnement SCIM, cliquez sur **Créer** pour terminer la configuration du fournisseur d'identité.

### Activer le provisionnement SCIM (Facultatif)
...
Cliquez sur **Créer** pour terminer la configuration du fournisseur d'identité.
