...
Ce manuel fournit des instructions étape par étape pour configurer le **Single Sign-On** (SSO) pour **PADS4** en utilisant **Microsoft Entra ID**.
...
1. **App Federation Metadata URL** (p. ex., `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/federationmetadata/2007-06/federationmetadata.xml?appid=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
2. **Certificate (Raw)** – Cliquez sur le lien pour le télécharger (p. ex., `your-application-name.cer`)
3. **Login URL** (p. ex., `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/saml2`)
4. **Microsoft Entra Identifier** (p. ex., `https://sts.windows.net/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/`)
...
4. **Metadata URL** – Entrez le **Microsoft Entra: App Federation Metadata URL** URL de métadonnées que vous avez collecté.
...
1. **Déconnectez-vous** et **actualisez** le PADS4 Portal.
