...
Pour spécifier quels groupes d'utilisateurs peuvent accéder à l'application, assignez-leur les groupes d'utilisateurs appropriés.
...
3. Cliquez sur **Users and groups**, choisissez le groupe que vous souhaitez assigner, puis cliquez sur **Select**.
...
Vous pouvez répéter cette action pour chaque groupe d'utilisateurs que vous souhaitez assigner à l'application.
...
1. **App Federation Metadata URL** (ex., `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/federationmetadata/2007-06/federationmetadata.xml?appid=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
2. **Certificate (Raw)** – Cliquez sur le lien pour le télécharger (ex., `your-application-name.cer`)
...
### Renseigner les détails du fournisseur

1. **Name** – Entrez un nom d'affichage pour le fournisseur dans PADS4
2. **Identifier** – Entrez le **Microsoft Entra Identifier** que vous avez collecté précédemment
...
1. **Provider Signing Certificate** - Téléversez le **Certificate (Raw)** (`.cer`) fichier que vous avez téléchargé depuis la configuration de votre fournisseur d'identité Microsoft Entra.
2. **PADS4 Identity Provider Certificate** - Téléversez un certificat (`.pfx`) pour signer les assertions SAML de PADS4 vers Microsoft Entra.
3. \*\*Mot de passe \*\*- Entrez le mot de passe pour le `.pfx` certificat téléversé (sous le champ du certificat).
...
Cliquez sur **Create** pour finaliser la configuration du fournisseur d'identité.
