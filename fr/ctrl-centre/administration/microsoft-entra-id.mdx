...
Pour configurer un fournisseur d'identité Microsoft Entra, vous devez vous connecter à Azure en tant qu'administrateur et créer une nouvelle application d'entreprise.

1. Accédez à [**https://portal.azure.com**](https://portal.azure.com) et connectez-vous en tant qu'administrateur.
2. Sur Azure, accédez à la page **Applications d'entreprise**&#x20;
...
7. Votre application est en cours de création.

## Configurer le Single Sign-On

Maintenant que votre application a été créée, vous devez activer et configurer le single sign-on.
...
<Warning>
  ...
  ex., pour un domaine nommé "domainone", utilisez [https://pads4.mycompany.com/rdx/nds.services.authentication.integration/api/v1/domainone/Saml2/Acs](https://pads4.mycompany.com/rdx/nds.services.authentication.integration/api/v1/domainone/Saml2/Acs)).
</Warning>
...
1. Vous devez modifier les **Attributs et revendications**. Cliquez sur **Modifier** pour ce faire.

   ![Download](/images/download-10.png)
2. Modifiez les **Attributs et revendications**
...
1. Cliquez sur **Ajouter une nouvelle revendication de groupe**.
2. Sélectionnez **Groupes affectés à l'application** (bouton radio).
3. Choisissez **ID de groupe** comme attribut source.
...
Pour spécifier quels groupes d'utilisateurs peuvent accéder à l'application, affectez-leur les groupes d'utilisateurs appropriés.
...
3. Cliquez sur **Utilisateurs et groupes**, choisissez le groupe que vous souhaitez affecter, puis cliquez sur **Sélectionner**.
...
Vous pouvez répéter cette action pour chaque groupe d'utilisateurs que vous souhaitez affecter à l'application.

## Collecter les données de configuration pour le fournisseur d'identité PADS4

Pour créer le fournisseur d'identité dans PADS4, rassemblez ces informations depuis votre configuration Microsoft Entra. Ouvrez la page **Single sign-on** et collectez :

1. **URL des métadonnées de fédération de l'application** (ex., `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/federationmetadata/2007-06/federationmetadata.xml?appid=xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`)
2. **Certificat (brut)** – Cliquez sur le lien pour le télécharger (ex., `your-application-name.cer`)
3. **URL de connexion** (p. ex., `https://login.microsoftonline.com/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/saml2`)
4. **Identifiant Microsoft Entra** (p. ex., `https://sts.windows.net/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx/`)

![Azure Set Up](/images/ctrl-centre/identity/AzureSetUp.png)

Vous utiliserez ces informations ultérieurement lors de la configuration du fournisseur d'identité dans PADS4.
...
3. **Reply URL** – Entrez l'URL de votre application PADS4 locale (p. ex., `https://pads4.mycompany.com`).
4. **Metadata URL** – Entrez l'**Microsoft Entra : URL des métadonnées de fédération d'application** URL des métadonnées que vous avez collectée.
...
5. L'**Identifier** et l'**Assertion Consumer Service URL** sont renseignés.

### Télécharger les certificats

1. **Provider Signing Certificate** - Téléchargez le **Certificate (Raw)** (`.cer`) fichier que vous avez téléchargé depuis votre configuration de fournisseur d'identité Microsoft Entra.
...
3. \*\*Mot de passe \*\*- Entrez le mot de passe pour le certificat `.pfx` téléchargé (sous le champ du certificat).
...
Vous pouvez activer le provisionnement SCIM lors de la création du fournisseur d'identité. Cette fonctionnalité synchronise les utilisateurs entre Microsoft Entra et PADS4, en créant, mettant à jour ou supprimant automatiquement les utilisateurs liés à l'application.
...
2. Le **SSO Login** bouton devrait maintenant être visible.
3. **Connectez-vous** à l'aide de vos identifiants Microsoft Entra.
