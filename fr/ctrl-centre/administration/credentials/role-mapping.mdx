...
Lorsque l'utilisateur se connectera, il sera affecté aux rôles pads4 en fonction du mappage suivant des groupes d'utilisateurs du fournisseur d'identité aux rôles pads4 que vous allez configurer.
...
* L'ID du groupe SSO doit correspondre à la valeur de revendication de groupe que vous avez configurée dans la configuration des revendications du fournisseur d'identité externe
  * *Par défaut, la valeur de revendication de groupe est :*
    * *L'ID de groupe d'utilisateur pour Azure*
    * *Le nom de groupe d'utilisateur pour Okta*
...
* Ce processus doit être répété autant que vous voulez spécifier différents groupes d'utilisateurs d'identité externe que vous souhaitez autoriser pour l'application.
...
* Ici, vous pouvez soit créer une nouvelle revendication, soit mettre à jour une revendication existante. Si vous créez une nouvelle revendication, vous devrez remplir les informations suivantes
  ...
  * Le **local attribute** – qui détermine la propriété qui sera remplie avec la valeur de revendication lorsque l'utilisateur pads sera créé
