...
* Allez sur [https://portal.azure.com/](https://portal.azure.com/) et connectez-vous avec un compte qui a des privilèges administratifs pour Microsoft Entra ID
...
* Utilisez les informations suivantes:
  ...
  * **Redirect URI:** L'URI de redirection spécifie où le serveur d'autorisation doit renvoyer l'utilisateur après une authentification réussie pour retourner un jeton d'accès à leur compte de messagerie.
    * **Type**: Web
...
* Allez à **Certificates & secrets** > **Client Secrets** > **Nouveau secret client**.
* Ajoutez une description (par exemple, "Secret SMTP") et sélectionnez une période d'expiration (par exemple, 24 mois).
* Cliquez**Add** , **puis copiez immédiatement** la **valeur** du secret client. **Remarque**: **Cette valeur n'est affichée qu'une seule fois**, et vous ne pourrez pas la récupérer après avoir quitté cette page. Si vous la perdez, vous devrez en générer une nouvelle. C'est votre "**ClientSecret**".
