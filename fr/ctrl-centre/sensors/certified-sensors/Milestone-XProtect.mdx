...
2. Accédez à la **Alarms** catégorie.
3. **Créez une ou plusieurs définitions d'alarme**.
4. **Spécifiez le déclencheur** qui déclenchera l'alarme (par ex., détection de ligne de franchissement, alerte de température).
...
1. Accédez à **User Mapping**.
2. Sélectionnez la **base de données Surveillance**.
3. Attribuez le **db\_owner** rôle (pour autoriser un accès complet).
...
4. Entrer le **adresse SQL Server, nom de la base de données, nom d'utilisateur (`PADS4Reader`) et mot de passe**.
...
Cette configuration permet **la récupération des alarmes en temps réel** depuis **Milestone XProtect** en utilisant **les notifications SQL Server**. L'intégration permet **la création automatisée d'incidents** dans PADS4 basée sur **les déclencheurs d'alarmes des caméras connectées**.
