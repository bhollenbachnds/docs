...
PADS4 ne se connecte pas directement à Milestone ; au lieu de cela, il **lit les données de la base de données SQL Server**. Cette approche permet à SQL Server d'**envoyer des notifications push** lorsque les données changent, améliorant ainsi les performances du système.
...
Un **SQL user account** est requis pour accéder à la base de données Milestone. Vous pouvez en créer un en utilisant **SSMS** (Interface Graphique Utilisateur) ou **SQL scripts**.

### **Option 1 : Créer un utilisateur SQL en utilisant SSMS**

1. **Se connecter au serveur SQL** via SSMS.
2. Développer **Security** > **Logins**.
3. **Clic droit** sur **Logins** et sélectionner **New Login**.
