...
2. Navigate to:\
   `Organization` → `Add-Ins` _`(or Apps in Exchange 2013)`_
...
3. Click **Upload custom apps** and select `App type` → `Office Add-in`
4. Click **Upload manifiest file (.xml) from device** and select the **manifest file** you saved earlier.
5. Follow the steps required **(User + Deployment)** to finish the configuration.
6. Once finished, the **PADS4 Workspace Add-In** will appear in the list of available apps.
...
<Note>
  Make sure the email addresses of the users who make use of the plugin have a workspace account.

  This account needs to use the same email address as the Outlook user; this is used for authentication with the PADS4 backend, which enables you to use the PADS4 Workspace Add-in.
</Note>