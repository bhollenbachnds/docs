...
HTML5 Viewers do not connect directly to data sources.

All Data Providers use the PADS4 Data Server.

### Content Library

Content stored in the PADS4 Content Library is supported.

Content referenced through local file system paths is not supported.

### Proof of Play

A maximum of 50,000 Proof of Play records can be stored locally while offline.

When the limit is reached, the oldest records are removed first.

### Storage Capacity

Available storage depends on the device hardware, browser implementation and available local storage.